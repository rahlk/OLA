...
def run_oracular_analysis_classification(app, slowdown):
    ...
    for method, ratio in sorted_method_ratio_list:
        ...
        if ratio <= optimal_ratio_threshold or "java.util" in method:
            special_cs_file.write(method + '\t' '2-object\n')
            two_object_sensitive_methods += 1
        elif ratio > optimal_ratio_threshold and "java.util" not in method:
...
def calculate_analysis_threshold(ratio_threshold):
    ...
    for method, ratio in sorted_method_ratio_list:
        if ratio <= ratio_threshold:
            if sens_method_cost_dict.__contains__(method) or "java.util" in method:
                analysis_weight += sens_method_cost_dict.get(method)
        else:
            if insens_method_cost_dict.__contains__(method) and "java.util" not in method:
